...
!
!     L. M. Thompson, 2021
!     OpenMP modifications added for performance optimization
!
      use mqc_gaussian
      use iso_fortran_env, only: int32, int64, real64
      use omp_lib  ! Added for OpenMP
!
!****x* Main/TDCIS
!*    NAME
...
!
      implicit none
      type(mqc_gaussian_unformatted_matrix_file)::fileInfo,temp_file,temp_file2
      character(len=:),allocatable::command,fileName,help_path,nucFile,increment_script,cap_unformatted_matrix
      ...
      real(kind=real64),allocatable::tcf_start,nucTime,eta
      ...
      type(mqc_twoERISet) :: eris_set
      type(mqc_twoERIs)::mo_ERIs
      type(mqc_scalar)::Vnn,final_energy,nIJ,pnIJ,hij,dij,wij,Remaining_e,CAP_State,total_sum
      type(mqc_determinant)::determinants
      type(mqc_scf_integral)::mo_core_ham,density,new_core_ham,core_ham

      type(mqc_scf_eigenvalues)::cap_eVals
      type(mqc_vector)::cap_eigvals,sum_eigvals
      type(mqc_matrix)::cap_eigvecs,sum_eigvecs
      ...
      type(mqc_matrix)::CI_Hamiltonian,exp_CI_Hamiltonian,CI_Overlap,iden,sh2AtMp,shlTyp,nPrmSh,prmExp,&
        conCoef,conCoTwo,shCoor,saved_pscf_amplitudes,newGeom,CI_Sum
      type(mqc_matrix),dimension(3)::CI_Dipole,dipole_eigvecs,Xmat,invXmat
      type(mqc_matrix)::Cmat,invCmat
      type(mqc_vector)::subs,nuclear_dipole,total_dipole,state_coeffs,td_ci_coeffs,field_vector,&
        td_field_vector,tcf_ci_epsilon,tcf,total_veldip,new_state_coeffs,chargeList,tmpPop
      type(mqc_vector),dimension(3)::dipole_eigvals
      type(mqc_vector),dimension(2)::nRoot

      type(mqc_matrix)::numerical_overlap,analytical_overlap,numerical_overlap_cap
      type(mqc_matrix)::pade_approx_cap_exp,numerical_overlap_cap_updated
      type(mqc_matrix)::CI_cap
      type(mqc_scf_integral)::cap_integral_MO,cap_integral
      
      real(16) :: max_val
      integer :: max_index
      
      type(mqc_matrix) :: CAP_tmp
...
!
      ...
      write(iOut,'(1X,A,I4,A)') 'OpenMP enabled with ', num_threads, ' threads'
      !$omp end master
      !$omp end parallel
!
!     Parse input options.
...
!********* section added to write out the state pops
          if(allocated(state_coeffs)) then
            ...
            write(10,'("Time step : ",F12.6)') current_time
...
!***************
        ...
        if(nucStep.ge.1) then
          ...
          call moleculeInfo%Nuclear_charges%print(6,'LMT nuclear charges')
...
!
        ...
        if(ci_string.eq.'oci') then
          ...
          if(nVirt.gt.min(int(wavefunction%nBasis-wavefunction%nAlpha),int(wavefunction%nBasis-Wavefunction%nBeta))) &
            ...
            6,'nVirt',nVirt,'wavefunction%nBasis-wavefunction%nAlpha',int(wavefunction%nBasis-wavefunction%nAlpha),&
...
!
      if(allocated(cap_unformatted_matrix)) then
        ...
        call temp_file2%getArray('NUMERICAL OVERLAP',numerical_overlap)
        call temp_file2%getArray('COMPLEX ABSORBING POTENTIAL',numerical_overlap_cap)
        call temp_file2%getESTObj('COMPLEX ABSORBING POTENTIAL',est_integral=cap_integral)
        call numerical_overlap%print(iOut,'NUMERICAL OVERLAP',Blank_At_Bottom=.true.)
        call numerical_overlap_cap%print(iOut,'CAP',Blank_At_Bottom=.true.)
        call numerical_overlap_cap%eigensys(eigenvals=cap_eigvals,reigenvecs=cap_eigvecs)
        call numerical_overlap_cap%diag(evals=cap_eigvals,evecs=cap_eigvecs)
        call cap_eigvals%print(iOut,'CAP Eigen values',Blank_At_Bottom=.true.)
        call cap_eigvecs%print(iOut,'CAP Eigen vectors',Blank_At_Bottom=.true.)
      endIf
!
...
!
        if(ci_string.eq.'oci') then
          ...
          do i = 1, 3
            ...
            call mqc_build_ci_hamiltonian(iOut, iPrint, CI_Hamiltonian=CI_Dipole(i),&
              Determinants=determinants, MO_Core_Ham=dipoleMO(i), SubsAIn=isubs)
          endDo
          !$omp end parallel do
          
          ! Sequential part for identity matrix operations
          do i = 1, 3
...
!         loop over pairs of Slater determinants and build transition density matrices. Get the H, N and Mu CI matrices
          ...
          do i = 1, numFile
            do j = 1, i
              ...
              !$omp critical(overlap_update)
              call CI_Overlap%put(nIJ,i,j,'hermitian')
              !$omp end critical(overlap_update)
              ...
              !$omp critical(hamiltonian_update)
              call CI_Hamiltonian%put(hIJ,i,j,'hermitian')
              !$omp end critical(hamiltonian_update)
              do k = 1, 3
                dij = get_dij(pnij,nullSize,rho(2),dipole(k))
                !$omp critical(dipole_update)
                call CI_Dipole(k)%put(dij,i,j,'hermitian')
                !$omp end critical(dipole_update)
              endDo
            endDo
          endDo
          !$omp end parallel do
          omp_end_time = omp_get_wtime()
          if(iPrint.ge.1) write(iOut,'(1X,A,F10.4,A)') 'NOCI matrix element time: ', &
            omp_end_time - omp_start_time, ' seconds'
...
!
        ...
        if(iPrint.ge.0) call wavefunction%pscf_amplitudes%print(iOut,'CI Eigenvectors',Blank_At_Bottom=.true.)
        if(iPrint.ge.0) call wavefunction%pscf_energies%print(iOut,'CI Eigenvalues',Blank_At_Bottom=.true.)
!
...
!
        ...
        !$omp end parallel do
        omp_end_time = omp_get_wtime()
        if(iPrint.ge.1) write(iOut,'(1X,A,F10.4,A)') 'Dipole diagonalization time: ', &
          omp_end_time - omp_start_time, ' seconds'

        do i = 1, 3
          if(iPrint.ge.0) write(iOut,'(1X,A)') 'Diagonalizing SD dipole axis '//trim(num2char(i))//NEW_LINE('A')
          ...
          if(iprint.ge.3) call dipole_eigvals(i)%print(iOut,'CI Dipole Eigenvalues axis '//trim(num2char(i)),&
            Blank_At_Bottom=.true.)
          if (ci_string.eq.'oci'.or.ci_string.eq.'ocas') then
            if(iPrint.ge.0) call mqc_print(matmul(matmul(dagger(wavefunction%pscf_amplitudes),CI_Dipole(i)),&
          ...
          elseIf(ci_string.eq.'noci') then
            if(iPrint.ge.2) call mqc_print(matmul(matmul(wavefunction%pscf_amplitudes%inv(),CI_Dipole(i)),&
              wavefunction%pscf_amplitudes),6,'State basis dipoles and transition dipoles on axis '//trim(num2char(i)),&
              Blank_At_Bottom=.true.)
          elseIf(allocated(cap_unformatted_matrix)) then
            if (ci_string.eq.'oci'.or.ci_string.eq.'ocas') then
              call CI_CAP%diag(cap_eigvals,cap_eigvecs)
            elseIf(ci_string.eq.'noci') then
              call CI_CAP%eigensys(CI_Overlap,cap_eigvals,cap_eigvecs)
            endIf
...
!
        ...
        do i = 0, maxsteps
          ...
          if (allocated(cap_unformatted_matrix)) then
            final_energy = get_CI_Energy(CI_Hamiltonian, td_ci_coeffs, CI_CAP)
          else
            final_energy = get_CI_Energy(CI_Hamiltonian, td_ci_coeffs)
          end if
          
          call CI_Overlap%identity(size(CI_Hamiltonian), size(CI_Hamiltonian))
          CI_Sum = CI_Hamiltonian
          if (allocated(cap_unformatted_matrix)) then
            CI_Sum=CI_Sum-imag*eta*CI_CAP
          end if
          do j = 1, 3
            CI_Sum=CI_Sum-(td_field_vector%at(j)*CI_Dipole(j))
          end do
          ...
          do j = 1, size(state_coeffs)
            CAP_State = sum_eigvecs%at(1,j)
            CAP_State = CAP_State * conjg(CAP_State)
            if (CAP_State%rval() .ge. max_val) then
              max_val = CAP_State%rval()
              max_index = j
            end if
          ...
          if (iPrint .ge. 0 .or. i .eq. maxsteps) then
            call final_energy%print(6, 'Energy (au)', Blank_At_Bottom = .true., &
                 FormatStr='(f14.8, " - ", f14.8, "i")')
          end if

          if(iPrint.ge.1.or.i.eq.maxsteps) call mqc_print(state_coeffs.outer.transpose(state_coeffs),6,'State Density matrix', &
            Blank_At_Bottom=.true.)
          if(ci_string.eq.'oci'.or.ci_string.eq.'ocas') then
            density = get_one_gamma_matrix(iOut,iPrint,td_ci_coeffs,determinants,&
              nOrbsIn=int(wavefunction%nBasis),subsain=isubs)
          ...
          Remaining_e = density%trace()
          write(*,'(A,F12.6)') 'Remaining e-=', Remaining_e%rval()
...
!
      function get_CI_Energy(CI_Hamiltonian,CI_vectors,CI_CAP) result(energy)
        ...
        type(mqc_matrix),optional, intent(in)::CI_CAP
        type(mqc_scalar)::energy
        energy=dot_product(matmul(dagger(CI_vectors),CI_Hamiltonian),CI_vectors)
        if (present(CI_CAP)) then
          energy =energy-imag*eta*dot_product(matmul(dagger(CI_vectors),CI_CAP),CI_vectors)
        end if
...
!
      function get_field_vector(dt,step,static_field,pulseShape,t0,omega,sigma,beta) result(td_field)
        ...
        case('cos 2')
          if (dt*step.ge.t0 .and. dt*step.lt.t0 + sigma) then
            td_field = static_field * (cos((pi / (2.0 * sigma)) * (dt*step - t0 - sigma)))**2 * &
                       cos(omega * (dt*step - t0))
          else if (dt*step.ge.t0 + sigma) then
            td_field = static_field * cos(omega * (dt*step - t0))
          else
            td_field = [0.0, 0.0, 0.0]
          endif
...
!
      subroutine get_rhos(rho,nIJ,pnIJ,nullSize,mo_I,mo_J,overlap,nBasis,nAlpha,nBeta)
        ...
        type(mqc_matrix)::mo_I_occ,mo_J_occ,mIJ,uMat,vMat,tmoI,tmoJ,rhoMat,tMat1,tMat2,tMat3,tMat4
...
!
      subroutine write_GauIn_file(iPrint,filename,matFile,doAppend,doProcMem,nProc,mem,route_addition,saveMat,&
        ...
        endIf
        if(doTwoERIs) then
          write(unitnumber,'(A,A,A)') '#P 6d 10f hf guess=read '//trim(geomcmd)//' scf=(conven,skip) geom=nocrowd &
            &int=noraf nosymm output=matrix'
        else
          write(unitnumber,'(A,A,A)') '#P 6d 10f hf guess=read '//trim(geomcmd)//' scf=(skip) geom=nocrowd &
            &int=noraf nosymm output=matrix'
        endIf
